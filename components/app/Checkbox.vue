...
<template>
    <label :for="id" :class="labelClasses" @click.stop="noop">
        ...
        <div :class="wrapperClasses" v-if="!slots.checked || !slots.unchecked">
            <div class="icon">
                <IconMinusIcon v-if="indeterminate" />
                <IconCheckIcon v-else />
            </div>
        </div>
...
.app-checkbox {
  ...
  &__wrapper {
    ...
    &:not(.app-checkbox__wrapper--default) {
      border-color: #CAD6E4;
    }

    &.app-checkbox__wrapper--default {
  ...
  .icon {
    ...
    &:not(.app-checkbox__wrapper) {
      border-color: white;
    }
  }

  &:checked {
    + .app-checkbox__wrapper > .icon {
      transform: scale(1);
      opacity: 1;
      border-color: white;
    }

    + .app-checkbox__wrapper--default > .icon {
      border-color: #CAD6E4;
    }
  }
  ...
  &:checked ~ .slot--unchecked {
    display: none;
  }

  &:not(:checked) ~ .slot--checked {
    display: none;
  }
}
</style>
